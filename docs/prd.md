...
2. Агент просить вставити 10-20 текстів або дати посилання на Notion-сторінку
...
### US-7: Notion як Content Hub
> "Все в одному місці"

**Як** творець контенту,
**я хочу** щоб агент читав з Notion і зберігав туди,
**щоб** мій workflow не розривався.

**Сценарій A — читання:**
1. Користувач: "напиши пост на основі моєї нотатки [Notion URL]"
2. Агент читає сторінку, витягує ключові ідеї
3. Генерує пост у стилі користувача на основі цього контенту

**Сценарій B — збереження:**
1. Після генерації: "зберегти в Notion? (y/n)"
2. Якщо так → пост з'являється в Notion Content Calendar з тегами: платформа, тема, дата, статус "draft"

**Acceptance Criteria:**
- Notion credentials через `.env` (NOTION_TOKEN, NOTION_DATABASE_ID)
- Якщо Notion не налаштований — агент працює з локальними файлами без помилок
- Читання Notion-сторінки підтримує: text, headings, lists, quotes
- Збереження додає метадані: platform, topic, created_at, status, profile_used
- Помилки Notion API не крашають агента — fallback на локальне збереження

**Priority:** P1

---

### US-8: Аналіз контенту
> "Що працює, а що ні?"
...
| `read_notion_page` | Коли користувач дає Notion URL | page_id | Page content as markdown |
| `write_to_notion` | Коли користувач каже "зберігай в Notion" | content + metadata | Notion page URL |
...
- **Залежностей мінімум:** anthropic, @notionhq/client, dotenv, chalk (для CLI output)
